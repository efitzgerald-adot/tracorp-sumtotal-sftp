...
# Remove Duplicates
def remove_duplicates(df_left, df_report):
    # Create blank df to paste into
    df = pd.DataFrame()

    try:
        # Convert subsets to same data types
        df_left['ActivityCode'] = df_left['ActivityCode'].str.replace(' ', '')
        df_report['ActivityCode'] = df_report['ActivityCode'].str.replace(' ', '')

        # Remove trailing whitespaces -per chat gpt
        df_left['EmployeeNumber'] = df_left['EmployeeNumber'].str.strip()
        df_report['EmployeeNumber'] = df_report['EmployeeNumber'].str.strip()
        
        df_left['ActivityCode'] = df_left['ActivityCode'].str.strip()
        df_report['ActivityCode'] = df_report['ActivityCode'].str.strip()

        # Convert dates
        df_left['DeleteThisDate'] = pd.to_datetime(df_left['RegistrationDate'])
        df_left['CompletionDate'] = df_left['RegistrationDate']

        df_report['DeleteThisDate'] = pd.to_datetime(df_report['RegistrationDate'])
        df_report['CompletionDate'] = df_report['RegistrationDate']

        # # Remove 'DeleteThisDate' column
        # df_left = df_left.drop(['DeleteThisDate'], axis=1)
        # df_report = df_report.drop(['DeleteThisDate'], axis=1)

        # df_left = df.drop(['RegistrationDate'], axis=1)
        # df_report = df.drop(['RegistrationDate'], axis=1)

        # # Restore RegistrationDate
        # df_left['RegistrationDate'] = df_left['CompletionDate']
        # df_report['RegistrationDate'] = df_report['CompletionDate']



        # Create a boolean mask to identify rows in df_left that match any row in df_report
        mask = df_left.apply(lambda row: any((df_report['EmployeeNumber'] == row['EmployeeNumber']) & 
            (df_report['ActivityCode'] == row['ActivityCode']) & 
            (df_report['CompletionDate'] == row['CompletionDate'])), axis=1)

        # Filter out rows in df_left that match any row in df_report
        df = df_left[~mask]

        print(df)

    except:
        print("Failure: remove_duplicates()")

    else:
        print("Sucess: remove_duplicate()")
...
def main():
    ...
    # Compare dfs & delete duplicates -per chat gpt
    df_merged = remove_duplicates(df_inputFile, df_inputReport)
